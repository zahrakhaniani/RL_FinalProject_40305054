...
def classify_transfer(record: dict, baseline: dict) -> dict:
    ...
    Decided on numbers, in this order: a scenario that ends up worse than
    scratch is negative regardless of how it started, otherwise early learning
    decides, and a difference smaller than the tolerance is neutral.
    """
    ...
    record_threshold = record["episodes_to_threshold"]
    baseline_threshold = baseline["episodes_to_threshold"]
    ...
    if final_delta < -CLASSIFY_TOLERANCE:
        verdict = "negative"
    elif early_delta > CLASSIFY_TOLERANCE or final_delta > CLASSIFY_TOLERANCE:
        verdict = "positive"
    elif early_delta < -CLASSIFY_TOLERANCE:
        verdict = "negative"
    else:
        verdict = "neutral"
