...
def plot_policy_difference(config: dict, payload: Optional[dict]) -> None:
    ...
    with np.load(map_file) as data:
        ...
        for row, (reward_mode, algorithm, grid) in enumerate(panels):
            ...
            for column, has_key in enumerate((0, 1)):
                ...
                wall_overlay(axis, env)
                axis.set_title(
                    f"{algorithm} vs VI, {reward_mode}, has_key={has_key}", fontsize=10
                )
                bare_axes(axis)
                mark_landmarks(axis, env)
    ...
    figure.suptitle("Policy difference map: model-free greedy action vs Value Iteration")
...
def write_transfer_table(payload: Optional[dict]) -> None:
        ...
        [
            {key: record.get(key) for key in
             ("variant", "scenario", "zero_shot_success", "early_success",
              "final_success", "episodes_to_threshold", "final_delta", "verdict")}
            for record in payload["records"]
        ]
    )
    ...
    grouped = (
        ...
        .agg(
            zero_shot=("zero_shot_success", "mean"),
            early=("early_success", "mean"),
            final=("final_success", "mean"),
            episodes_to_80pct=("episodes_to_threshold", "mean"),
            vs_scratch=("final_delta", "mean"),
            verdict=("verdict", lambda values: values.mode().iat[0]),
        )
        .reset_index()
        .round(3)
    )
    write_markdown_table(grouped, paths.RAW_DATA / "transfer_summary.md")
